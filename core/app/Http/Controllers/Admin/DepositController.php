...
class DepositController extends Controller
{
    ...
    public function pending()
    {
        ...
        $deposits = Deposit::where('method_code', '>=', 1000)->where('status', 2)->with(['user', 'gateway', 'host'])->orderBy('id','desc')->paginate(getPaginate());
    ...
    public function approved()
    {
        ...
        $deposits = Deposit::where('method_code','>=',1000)->where('status', 1)->with(['user', 'gateway', 'host'])->orderBy('id','desc')->paginate(getPaginate());
        return view('admin.deposit.log', compact('pageTitle', 'emptyMessage', 'deposits'));
    }
    ...
    public function successful()
    {
        ...
        $deposits = Deposit::where('status', 1)->with(['user', 'gateway', 'host'])->orderBy('id','desc')->paginate(getPaginate());
        return view('admin.deposit.log', compact('pageTitle', 'emptyMessage', 'deposits'));
    }
    ...
    public function rejected()
    {
        ...
        $deposits = Deposit::where('method_code', '>=', 1000)->where('status', 3)->with(['user', 'gateway', 'host'])->orderBy('id','desc')->paginate(getPaginate());
        return view('admin.deposit.log', compact('pageTitle', 'emptyMessage', 'deposits'));
    }
    ...
    public function deposit()
    {
        ...
        $deposits = Deposit::with(['user', 'gateway', 'host'])->where('status','!=',0)->orderBy('id','desc')->paginate(getPaginate());
    ...
    public function depositViaMethod($method,$type = null){
        ...
        if ($type == 'approved') {
            ...
            $deposits = Deposit::where('method_code','>=',1000)->where('method_code',$method->code)->where('status', 1)->orderBy('id','desc')->with(['user', 'gateway', 'host']);
        }elseif($type == 'rejected'){
            $pageTitle = 'Rejected Payment Via '.$method->name;
            $deposits = Deposit::where('method_code','>=',1000)->where('method_code',$method->code)->where('status', 3)->orderBy('id','desc')->with(['user', 'gateway', 'host']);
            ...
            $deposits = Deposit::where('method_code','>=',1000)->where('method_code',$method->code)->where('status', 2)->orderBy('id','desc')->with(['user', 'gateway', 'host']);
        }else{
            $pageTitle = 'Payment Via '.$method->name;
            $deposits = Deposit::where('status','!=',0)->where('method_code',$method->code)->orderBy('id','desc')->with(['user', 'gateway', 'host']);
        }
    ...
    public function details($id)
    {
        ...
        $details = Transaction::where('deposit_trx', $deposit->trx)->firstOrFail();
        $pageTitle = $deposit->user->username.' deposited ' . showAmount($deposit->amount) . ' '.$general->cur_text;
